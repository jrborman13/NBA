...
@st.cache_data(ttl=3600, show_spinner=False)
def _fetch_offensive_synergy_playtype(playtype: str, season: str) -> tuple[str, pd.DataFrame]:
    """Fetch one offensive synergy playtype — called concurrently by the bulk fetcher."""
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                ...
                print(f"Error fetching bulk offensive synergy for playtype {playtype}: {e}")
                return playtype, pd.DataFrame()
    return playtype, pd.DataFrame()
...
def get_all_players_offensive_synergy_bulk(season: str = CURRENT_SEASON) -> Dict[str, pd.DataFrame]:
    ...
    Makes 11 API calls (one per playtype) run concurrently with up to 4 workers,
    reducing wall-clock time from ~50 s to ~12–15 s.
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for i, playtype in enumerate(SYNERGY_PLAYTYPES):
            time.sleep(i * 0.2)  # 0.2 s stagger per submission
            futures[executor.submit(_fetch_offensive_synergy_playtype, playtype, season)] = playtype

        for future in as_completed(futures):
            playtype, df = future.result()
            result[playtype] = df
...
@st.cache_data(ttl=3600, show_spinner=False)
def _fetch_defensive_synergy_playtype(playtype: str, season: str) -> tuple[str, pd.DataFrame]:
    """Fetch one defensive synergy playtype — called concurrently by the bulk fetcher."""
    for attempt in range(3):
        try:
            df = endpoints.SynergyPlayTypes(
                league_id=LEAGUE_ID,
            ...
            if attempt < 2:
                ...
                print(f"Error fetching bulk defensive synergy for playtype {playtype}: {e}")
                return playtype, pd.DataFrame()
    return playtype, pd.DataFrame()


def get_all_teams_defensive_synergy_bulk(season: str = CURRENT_SEASON) -> Dict[int, Dict[str, pd.DataFrame]]:
    """
    Fetch defensive synergy for ALL teams in 11 API calls (one per playtype),
    run concurrently with up to 4 workers.
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for i, playtype in enumerate(SYNERGY_PLAYTYPES):
            time.sleep(i * 0.2)  # 0.2 s stagger per submission
            futures[executor.submit(_fetch_defensive_synergy_playtype, playtype, season)] = playtype

        for future in as_completed(futures):
            playtype, df = future.result()
            if len(df) > 0 and 'TEAM_ID' in df.columns:
                for team_id, team_df in df.groupby('TEAM_ID'):
                    tid = int(team_id)
                    if tid not in result:
                        result[tid] = {}
                    result[tid][playtype] = team_df

    return result


def get_opponent_defensive_synergy_from_bulk(
    team_id: int,
    bulk_defensive_synergy: Dict[int, Dict[str, pd.DataFrame]]
) -> Dict[str, pd.DataFrame]:
    """Extract a single team's defensive synergy from the bulk dict."""
    return bulk_defensive_synergy.get(int(team_id), {})
