...
Turned the hexagon weights from face-valid-only into backtest-validated, using `analysis/hexagon_weight_backtest.py` as the referee (grades any `{sub_metric: weight}` for an axis vs a FROZEN, independent, on-court impact outcome; no leakage; gravity/finishing/shooting judged temporally N→N+1; tune on TRAIN 2021-22..23-24, judge once on TEST 2024-25..25-26). Referee objects (built by its `REBUILD_SQL`): **`hex_weight_outcomes`** + staging **`hex_shot_own_stage`** (own shots rim-vs-jumper from `shot_event.shooter_id`) and **`hex_oreb_stage`** (OREB on-court from `possession_lineup`). Search harness: `analysis/hexagon_weight_search.py` (grid on TRAIN, judged on TEST, leaderboard face-validity gate). **Shipped 2 of 6 axes** via one `UPDATE hexagon_weights` (dropped sub-metrics set to weight 0, reversible; `v_player_hexagon` skips 0-weight terms): **finishing** drop `team_rim_freq_lift` → TEST partial **0.446→0.500** (rim-running bigs on top); **playmaking** lead on `ast_pts_created`, drop `ast_pct`+`drive_ast` → **0.151→0.196** (elite creators; Jokić/CP3 rise). Both beat current AND equal, face-validity confirmed. **Kept 4 axes unchanged with reasons:** defending (best alt +0.008 but drops Wembanyama → loses rim protection), gravity (only pure `shot_diet_gravity_efg` = temporal near-dup of its own outcome; role-player leaderboard), shooting (low year-to-year ceiling; gain over equal is noise), rebounding (best "gain" is `sc_rate_on_minus_off` = **leakage**, a near-duplicate of the outcome's OREB half; excluding it nothing beats the baselines — referee compromised, flagged). `sql/hexagon_weights.sql` seed synced; detail in `SUPABASE_DERIVED_OBJECTS.md` §4.
