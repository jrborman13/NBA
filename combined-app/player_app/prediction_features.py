...
def get_all_prediction_features(
    ...
    bulk_defensive_synergy: Optional[Dict[int, Dict[str, pd.DataFrame]]] = None,
    ...
    try:
        ...
        # Fetch opponent defensive synergy data (use bulk if provided — avoids 11 API calls per team)
        if bulk_defensive_synergy is not None:
            opp_defense_data = psyn.get_opponent_defensive_synergy_from_bulk(
                opponent_team_id, bulk_defensive_synergy
            )
        else:
