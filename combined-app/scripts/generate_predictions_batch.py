...
def generate_predictions_for_date(game_date: date, output_dir: str = None, exclude_injured: bool = True,
                                  ...
                                  tipoff_time_filter: str = None, simulation_config=None, progress_callback=None):
    """
    ...
    for game_idx, matchup in enumerate(matchups, 1):
        ...
        try:
            ...
            def _inner_progress(current, total, player_name):
                if current % 5 == 0 or current == total:
                    print(f"  Progress: {current}/{total} ({player_name})")
                if progress_callback is not None:
                    progress_callback(game_idx, total_games, current, total, player_name, matchup_key)
            ...
            for player_id, player_data in all_predictions.items():
                ...
                _is_home_p = player_data.get('is_home', False)
