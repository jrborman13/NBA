...
def get_player_zone_percentiles(player_id, player_stats_df, min_games=10, min_zone_fga=10):
    """
    League percentile rank (0-100, higher = more) of a player's frequency and FG% in each
    shooting zone, for the zone-shooting cards.

    - Frequency percentile is ranked within the games-qualified pool (GamesPlayed >= min_games)
      so deep-bench noise doesn't distort the field.
    - FG% (accuracy) percentile is ranked only among pool players who actually attempt that zone
      (zone FGA >= min_zone_fga), so a 0%/100%-on-one-shot line doesn't skew the scale.

    Returns {zone_key: {'freq_pctile': int|None, 'acc_pctile': int|None}} for
    zone_key in rim/smr/lmr/c3/atb3, or None if the player isn't in the data.
    """
    pid = str(player_id)
    prow = player_stats_df[player_stats_df['PlayerId'] == pid]
    if len(prow) == 0:
        return None

    pool = player_stats_df
    if 'GamesPlayed' in pool.columns:
        pool = pool[pool['GamesPlayed'] >= min_games]

    # zone_key -> pbpstats column prefix
    zones = {'rim': 'AtRim', 'smr': 'ShortMidRange', 'lmr': 'LongMidRange', 'c3': 'Corner3', 'atb3': 'Arc3'}

    def _pctile(value, series):
        """percent_rank: share of the pool strictly below `value`, 0-100 (matches the hexagon convention)."""
        series = series.dropna()
        if pd.isna(value) or len(series) < 2:
            return None
        return int(round((series < value).sum() / (len(series) - 1) * 100))

    out = {}
    for zk, prefix in zones.items():
        freq_col, acc_col, fga_col = f'{prefix}Frequency', f'{prefix}Accuracy', f'{prefix}FGA'
        res = {'freq_pctile': None, 'acc_pctile': None}
        if freq_col in pool.columns:
            res['freq_pctile'] = _pctile(prow[freq_col].values[0], pool[freq_col])
        if acc_col in pool.columns and fga_col in pool.columns:
            attempters = pool[pool[fga_col] >= min_zone_fga]
            res['acc_pctile'] = _pctile(prow[acc_col].values[0], attempters[acc_col])
        out[zk] = res
    return out
