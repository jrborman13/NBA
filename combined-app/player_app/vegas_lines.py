...
# Load .env so the API key is available regardless of which page imports this module
try:
...
def save_lines(lines: Dict):
    ...
    # Fallback to JSON (best-effort; silently skipped on read-only filesystems)
    try:
        with open(LINES_FILE, 'w') as f:
            json.dump(lines, f, indent=2)
    except OSError:
        logger.warning("Could not write player_lines.json (read-only filesystem). Lines not persisted locally.")
