...
class SimulationConfig:
    """
    Configuration for Monte Carlo simulation-based ceiling/floor estimation.

    Each iteration perturbs projected_minutes, opponent defensive rating, pace,
    and usage rate by sampling from Gaussian distributions centered on the base value.
    Ceiling = config.ceiling_percentile of simulated FPTS distribution.
    Floor   = config.floor_percentile of simulated FPTS distribution.

    Attributes:
        enabled:                Run simulation; if False, fall back to historical percentiles.
        n_iterations:           Number of Monte Carlo samples (100–1000).
        ceiling_percentile:     Percentile used for ceiling (default 90).
        floor_percentile:       Percentile used for floor (default 10).
        minutes_std:            Std dev for minute sampling. None = use player's own historical std.
        opp_def_rtg_noise_std:  Gaussian noise std added to opponent DefRtg each iteration.
        pace_noise_std:         Gaussian noise std added to opponent Pace each iteration.
        usage_noise_std:        Gaussian noise std (pct points) added to usage rate each iteration.
    """
    enabled: bool = True
    n_iterations: int = 500
    ceiling_percentile: float = 90.0
    floor_percentile: float = 10.0
    minutes_std: Optional[float] = None   # None → derive from player's historical minute std
    opp_def_rtg_noise_std: float = 3.0
    pace_noise_std: float = 2.0
    usage_noise_std: float = 2.0
...
class PlayerStatPredictor:
    ...
    def predict_stat(
        ...
        # Cache season PPG once — used in multiple places (3PT check, fatigue, regression, cap)
        season_ppg = player_features.get('season_ppg', rolling_avgs.get('Season', {}).get('PTS', 0.0)) or rolling_avgs.get('Season', {}).get('PTS', 0.0)
        ...
        # Dynamic vs_opponent weighting based on sample size:
        #   3+ games: full weight (35/25/20/20) — signal is reliable
        #   2 games:  reduced weight (35/30/25/10) — small sample, blend back toward season/L5
        #   0-1 games: no vs_opponent weight (45/30/25/0)
        if vs_opp_games >= 3:
            # Enough history — use full weights
            base_prediction = (
            ...
            # Small sample — reduce vs_opponent weight from 20% to 10%, redistribute to L5/season
            base_prediction = (
                0.35 * weighted_avg +
                0.30 * l5_avg +
                0.25 * season_avg +
            ...
            # No opponent history - redistribute weight to recents and season
            base_prediction = (
                0.45 * weighted_avg +
                0.30 * l5_avg +
                0.25 * season_avg
        ...
        pct_pts_from_3s_check = (pts_from_3s_check / season_ppg * 100) if season_ppg > 0 else 0.0
        ...
        if season_ppg < 27 and avg_minutes > 0 and recent_minutes > avg_minutes * 1.15:  # Playing 15%+ more than season avg
        ...
        # Usage rate modifier for PTS predictions
        # USG_PCT measures how often a player is involved in possessions.
        # High-usage players (>28%) slightly outperform raw averages on good nights;
        # low-usage role players (<18%) are more opportunity-dependent.
        # ±3% is intentionally small — big enough to meaningfully rank players for DK but
        # small enough not to distort individual game predictions.
        if stat == 'PTS':
            usg_pct = player_features.get('usage_rate', 20.0)
            if usg_pct > 28.0:
                adjusted_prediction *= 1.03
                factors['usage_rate'] = f"High usage ({usg_pct:.1f}%) +3%"
            elif usg_pct < 18.0:
                adjusted_prediction *= 0.97
                factors['usage_rate'] = f"Low usage ({usg_pct:.1f}%) -3%"
        ...
        if len(game_logs) > 0:
            ...
            if all(col in game_logs.columns for col in required_cols):
                ...
                if len(fpts_values) >= 5:  # Need at least 5 games for meaningful percentiles
                    # Calculate all three percentiles in one call (25th, 50th, 75th)
                    floor_fpts, median_fpts_hist, ceiling_fpts = (
                        float(v) for v in np.percentile(fpts_values, [25, 50, 75])
                    )
...
def _perturb_features(
    ...
    """
    Return a shallow copy of player_features with key inputs perturbed for one MC iteration.
    Only the scalar keys that drive prediction variance are copied/modified; all other
    keys (DataFrames, nested dicts not being changed) are referenced directly to keep
    the copy cheap (< 1 ms per call).
    """
    import copy as _copy
    pf = dict(player_features)  # shallow top-level copy

    # --- Minutes ---
    rolling_avgs = player_features.get('rolling_avgs', {})
    avg_minutes = rolling_avgs.get('Season', {}).get('MIN', 0.0)
    if avg_minutes > 0:
        if config.minutes_std is not None and config.minutes_std > 0:
            min_std = config.minutes_std
        else:
            game_logs = player_features.get('game_logs', pd.DataFrame())
            if len(game_logs) >= 5 and 'MIN' in game_logs.columns:
                min_std = float(game_logs['MIN'].head(15).std())
                min_std = max(min_std, 1.0)  # floor at 1 minute std
            else:
                min_std = avg_minutes * 0.15  # fallback: 15% of avg
        perturbed_min = float(rng.normal(avg_minutes, min_std))
        pf['projected_minutes'] = max(5.0, min(48.0, perturbed_min))

    # --- Opponent stats (def_rating, pace) ---
    opp = _copy.copy(player_features.get('opponent', {}))
    if config.opp_def_rtg_noise_std > 0:
        base_def = opp.get('def_rating', 110.0)
        opp['def_rating'] = max(95.0, min(125.0, float(rng.normal(base_def, config.opp_def_rtg_noise_std))))
    if config.pace_noise_std > 0:
        base_pace = opp.get('pace', 100.0)
        opp['pace'] = max(88.0, min(115.0, float(rng.normal(base_pace, config.pace_noise_std))))
    pf['opponent'] = opp

    # --- Usage rate ---
    if config.usage_noise_std > 0:
        base_usg = player_features.get('usage_rate', 20.0)
        pf['usage_rate'] = max(5.0, min(45.0, float(rng.normal(base_usg, config.usage_noise_std))))
...
def run_monte_carlo_simulation(
    ...
    """
    Run N Monte Carlo iterations to compute simulation-based ceiling/floor/variance.

    Data is fetched ONCE (already in player_features); this function only runs the
    lightweight scoring math (predict_all_stats) with perturbed inputs each time.

    Returns:
        Dict with ceiling, floor, median, variance, std_dev, n_simulations, simulation_used
    """
    rng = np.random.default_rng()
    fpts_samples = np.empty(config.n_iterations)

    for i in range(config.n_iterations):
        perturbed = _perturb_features(player_features, rng, config)
        sim_preds = predictor.predict_all_stats(perturbed)
        fpts_samples[i] = sim_preds.get('FPTS', Prediction('FPTS', 0.0, 'low', {}, {})).value

    return {
        'ceiling':       round(float(np.percentile(fpts_samples, config.ceiling_percentile)), 1),
        'floor':         round(float(np.percentile(fpts_samples, config.floor_percentile)), 1),
        'median':        round(float(np.percentile(fpts_samples, 50)), 1),
        'variance':      round(float(np.var(fpts_samples)), 2),
        'std_dev':       round(float(np.std(fpts_samples)), 2),
        'n_simulations': config.n_iterations,
...
def generate_prediction(
    ...
    bulk_defensive_synergy: Optional[Dict] = None,
    ...
    player_features = features.get_all_prediction_features(
        ...
        bulk_defensive_synergy=bulk_defensive_synergy,
    ...
    # Calculate ceiling/floor: use Monte Carlo simulation if configured, else historical percentiles
    sim_cfg = simulation_config if simulation_config is not None else SimulationConfig()
    if sim_cfg.enabled:
        ceiling_floor = run_monte_carlo_simulation(predictor, player_features, sim_cfg)
    else:
...
def generate_predictions_for_game(
    ...
    # Fetch ALL bulk data upfront — single-call endpoints run concurrently,
    # then synergy (internally parallelized, 4 workers each) runs sequentially
    # to avoid stacking too many connections on stats.nba.com.
    ...
    # Phase 1: run all single-call endpoints concurrently (max 5 workers)
    with ThreadPoolExecutor(max_workers=5) as _ex:
        _f_game_logs    = _ex.submit(features.get_bulk_player_game_logs)
        _f_advanced     = _ex.submit(features.get_bulk_player_advanced_stats)
        _f_drives       = _ex.submit(ds.get_all_player_drives_stats)
        _f_team_stats   = _ex.submit(features_module.get_bulk_team_stats)
        _f_team_stats_l5= _ex.submit(features_module.get_bulk_team_stats_last_n, 5)
        _f_player_misc  = _ex.submit(ms.get_player_misc_stats)
        _f_team_misc    = _ex.submit(ms.get_team_misc_stats)

        bulk_game_logs       = _f_game_logs.result()
        bulk_advanced_stats  = _f_advanced.result()
        bulk_drives_stats    = _f_drives.result()
        bulk_team_stats      = _f_team_stats.result()
        bulk_team_stats_L5   = _f_team_stats_l5.result()
        # misc results just warm caches; return values unused

    # Phase 2: synergy (each internally uses 4 parallel workers)
    bulk_offensive_synergy  = features.get_cached_bulk_offensive_synergy()
    bulk_defensive_synergy  = psyn.get_all_teams_defensive_synergy_bulk()

    bulk_fetch_total = time.time() - bulk_fetch_start
    print(f"  [Bulk fetch] completed in {bulk_fetch_total:.1f}s")
...
def calculate_edge(prediction: float, line: float) -> Tuple[float, float, str]:
    ...
    if edge >= 1.5 or edge_pct >= 30:
        lean = "Strong Over"
    elif edge >= 0.5 or edge_pct >= 15:
        lean = "Lean Over"
    elif edge <= -1.5 or edge_pct <= -30:
        lean = "Strong Under"
    elif edge <= -0.5 or edge_pct <= -15:
        lean = "Lean Under"
    else:
