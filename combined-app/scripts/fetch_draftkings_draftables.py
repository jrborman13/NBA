...
def fetch_draftgroup_info(draftgroup_id: int) -> Optional[Dict]:
    """
    Fetch draftgroup metadata from DraftKings API, including the games list
    (each game has startDate in UTC, description like "CHI @ GSW").

    Endpoint: https://api.draftkings.com/draftgroups/v1/{draftgroup_id}?format=json

    Returns:
        draftGroup dict on success, None on failure.
    """
    url = f"https://api.draftkings.com/draftgroups/v1/{draftgroup_id}?format=json"
    headers = get_headers()
    try:
        response = requests.get(url, headers=headers, timeout=15)
        if response.status_code == 200:
            data = response.json()
            return data.get('draftGroup')
        else:
            print(f"fetch_draftgroup_info: status {response.status_code}")
            return None
    except Exception as e:
        print(f"fetch_draftgroup_info error: {e}")
        return None
