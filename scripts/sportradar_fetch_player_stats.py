#!/usr/bin/env python3
"""
Fetch NBA Player Stats from Sportradar API and Store in Supabase
Fetches player advanced stats and stores in sr_player_stats table.
"""
...
def fetch_and_store_player_stats():
    """Fetch player stats from Sportradar NBA API and store in Supabase"""
    print(f"[{datetime.now()}] Starting Sportradar player stats fetch for season {CURRENT_SEASON}")
    ...
    try:
        print("Fetching player advanced stats from Sportradar NBA API...")
        
        season_year = CURRENT_SEASON.split('-')[0]
        endpoint = f"seasons/{season_year}/players/statistics"
        
        # #region agent log
        debug_log(session_id, run_id, "A", "sportradar_fetch_player_stats.py:fetch_and_store_player_stats",
        ...
        if isinstance(response_data, list):
            ...
            data = response_data.get('players', response_data.get('data', response_data.get('statistics', [])))
        else:
            data = []
        ...
        supabase.table('sr_player_stats').upsert({
