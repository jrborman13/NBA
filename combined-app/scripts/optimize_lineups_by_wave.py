...
def optimize_combined_waves(game_date: date, draftables_path: str, selected_wave_times: List[str],
                           ...
                           team_fpts_scalars: Optional[Dict] = None,
    ...
    # Apply implied total scalars to FPTS
    if team_fpts_scalars and 'Team' in merged_df.columns:
        scalars = merged_df['Team'].map(team_fpts_scalars).fillna(1.0)
        merged_df['FPTS'] *= scalars
        if 'FPTS_Ceiling' in merged_df.columns:
            merged_df['FPTS_Ceiling'] *= scalars
        if 'FPTS_Floor' in merged_df.columns:
            merged_df['FPTS_Floor'] *= scalars
