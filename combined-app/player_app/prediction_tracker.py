...
def log_prediction(record: PredictionRecord):
    ...
    # Fallback to CSV (best-effort; silently skipped on read-only filesystems)
    try:
        ...
        logger.warning("Could not write predictions_log.csv (read-only filesystem). Prediction not persisted locally.")
...
def update_actual_result(
    ...
    try:
        ...
        logger.warning("Could not update predictions_log.csv (read-only filesystem).")
    return False
