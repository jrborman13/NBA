...
def get_similar_players(
    ...
    # Pre-build index for O(1) lookup instead of a full DataFrame scan per player
    player_index = {row['PLAYER_ID']: row for _, row in features_df.iterrows()}
    ...
    for other_id, similarity in player_similarities.items():
        ...
        other_row = player_index.get(other_id)
